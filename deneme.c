...
int main()
{
	...
	*/
	...
	{
        temp = m / (int)pow(10, digits - 1);
        printf("%d ", temp);
		printf("mbaslangic=%d\n",m);
		printf("%d",(int)pow(10, digits - 1));
		m = m%(int)pow(10, digits - 1);
		printf("mson=%d\n",m);
        digits--;
    }
    
    printf("\narithmetic mean of ones and hundreds place: %.1f", (float)((n % 10) + ((n / 100) % 10)) / 2);   //birler ve yüzler basamağının ortalamasını yazdır
